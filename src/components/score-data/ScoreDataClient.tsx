...
function normalizeAllBuckets(
  ...
  for (let s = 0; s <= scoreMax; s += bucketSize) {
    buckets.push({
      start: s,
      end: Math.min(s + bucketSize - 1, scoreMax),
...
function estimateMean(buckets: Bucket[], total: number) {
  if (!total) return 0;
  return (
    buckets.reduce((sum, b) => sum + ((b.start + b.end) / 2) * b.count, 0) /
    total
  );
}
...
function buildScoreData(payload: any): ScoreData | null {
  ...
  const bucketSize = Math.max(
    1,
    Math.round(Number(payload.bucketSize) || SCORE_BUCKET_SIZE),
  );
  const scoreMax = Math.max(
    bucketSize,
    Math.round(Number(payload.scoreMax) || SCORE_MAX),
  );
  const buckets = normalizeAllBuckets(payload.buckets, bucketSize, scoreMax);
  const counted = buckets.reduce((s, b) => s + b.count, 0);
  const total = Math.max(
    0,
    Math.round(Number(payload.totalEntries) || counted),
  );
  ...
  return {
    ...
    buckets,
    chartBuckets: buckets.filter(
      (b) =>
        b.start >= SCORE_CHART_MIN_START && b.start <= SCORE_CHART_MAX_START,
    ),
  };
}
...
export default function ScoreDataClient() {
  ...
  const higherRange =
    data && compareScore !== null
      ? estimateHigherRange(data, compareScore)
      : null;
  const percentileRange =
    data && higherRange && data.totalEntries > 0
      ? {
          min:
            (Math.max(0, data.totalEntries - higherRange.max) /
              data.totalEntries) *
            100,
          max:
            (Math.max(0, data.totalEntries - higherRange.min) /
              data.totalEntries) *
            100,
        }
      : null;
  ...
  const maxCount = data
    ? Math.max(1, ...data.chartBuckets.map((b) => b.count))
    : 1;
    ...
    <main className="pb-[72px]">
      ...
      >
        ...
        <p className="relative font-mono text-[11px] font-medium text-ash uppercase tracking-[0.1em] mb-[14px]">
          <span
            className="inline-block rounded-full bg-ink mr-[9px]"
            style={{ width: "7px", height: "7px", verticalAlign: "1px" }}
          />
          BITSAT · Score data · 1500+ datapoints
        </p>
        <h1
          className="relative font-sans text-ink m-0"
          style={{
      ...
      {/* ── DATASET TABS ── */}
      <section
        className="rounded-xl border border-black/10 bg-sand mb-[18px]"
            ...
            return (
              <button
                ...
                className={`text-left grid items-center gap-[6px] min-h-[82px] p-[16px_18px] rounded-[10px] border transition-all duration-[180ms] cursor-pointer ${
                  active ? "border-ink bg-ink" : "border-black/[0.16] bg-bg/60"
                }`}
              >
                <div
                  className={`flex items-center gap-[9px] font-mono font-bold text-[14px] ${active ? "text-bg" : "text-ink"}`}
                >
                  <span
                ...
                <div
                  className={`font-mono text-[12px] ${active ? "text-bg/70" : "text-tan"}`}
                >
      ...
      >
        ...
        <div className="border border-black/[0.12] rounded-xl bg-bg p-[18px] grid gap-3">
          <div className="flex items-baseline justify-between gap-[14px] flex-wrap">
            ...
            <span className="font-serif text-[13px] text-tan">
              enter total marks
            </span>
          </div>
          <div className="flex items-end justify-between gap-[14px] min-h-[92px] p-[16px_18px] border border-black/10 rounded-xl bg-bg">
              ...
              onChange={(e) => {
                const v =
                  e.target.value === ""
                    ...
                    : Math.max(
                        0,
                        Math.min(SCORE_MAX, Math.round(Number(e.target.value))),
                      );
                setCompareScore(v);
              }}
            ...
            />
            <span
              className="shrink-0 pb-2 text-tan font-normal whitespace-nowrap"
              style={{
        ...
        >
          ...
          <div className="px-3 py-2 rounded-pill border border-bg/20 bg-bg/[0.08] text-bg/70 font-mono text-[12px] font-medium leading-[1.15] w-fit">
            {config.label} ·{" "}
            {data
              ? `${formatNumber(data.totalEntries)} bucketed entries`
              : "loading..."}
          </div>
      ...
      >
        <div className="flex justify-between items-start gap-5 flex-wrap mb-[22px]">
          <div style={{ minWidth: "min(100%, 520px)", maxWidth: "760px" }}>
            ...
            <h2
              className="font-serif font-normal text-ink mt-2 m-0"
              style={{
            ...
            <p
              className="font-serif text-ash leading-[1.58] mt-3 m-0"
              style={{
                fontSize: "clamp(.96rem,1.3vw,1.08rem)",
                maxWidth: "66ch",
              }}
            >
              {loading ? "" : config.copy}
        ...
        {data && !loading && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-[18px] mb-[22px]">
            {[
              {
                label: "Above 300 est.",
                value: above300 ? formatRange(above300) : "--",
              },
              {
                label: "Above 270 est.",
                value: above270 ? formatRange(above270) : "--",
              },
              {
                label: "Mean est.",
                value: `~ ${formatScoreStat(data.meanScore)}`,
              },
              { label: "Median bucket", value: data.medianRange || "--" },
            ].map((m) => (
              <div
                key={m.label}
                className="border border-black/[0.12] rounded-xl bg-bg p-4 min-w-0"
              >
                <span className="block text-tan font-mono text-[10px] font-medium uppercase tracking-[0.12em]">
                  {m.label}
                </span>
                <strong
                  className="block mt-2 text-ink font-normal"
                  style={{
        ...
        {data && !loading && (
          <article className="border border-black/[0.12] rounded-xl bg-bg overflow-hidden mt-6">
            <div className="flex items-center justify-between gap-3 flex-wrap p-[18px_20px] border-b border-black/10 bg-cream">
              <h3
                className="m-0 text-ink font-serif font-normal"
                style={{
              ...
              <span className="inline-flex items-center justify-center gap-[7px] px-[13px] py-2 rounded-pill bg-ink border border-ink text-bg font-mono text-[12px] font-semibold">
                {config.label} · {formatNumber(data.totalEntries)} bucketed
                entries
              </span>
            </div>

            <div
              className="overflow-x-auto overflow-y-visible py-[22px]"
            ...
            >
              <div
                ...
                style={{
                  ...
                  backgroundImage:
                    "linear-gradient(to top, rgba(15,14,11,0.08) 1px, transparent 1px)",
              ...
              >
                {data.chartBuckets.map((b, i) => {
                  const height = Math.max(
                    b.count > 0 ? 2 : 0,
                    Math.round((b.count / maxCount) * 100),
                  );
                  const label = i % 2 === 0 ? String(b.start) : "";
                  const isCompare =
                    compareScore !== null &&
                    compareScore >= b.start &&
                    compareScore <= b.end;

                  return (
                    <div
                      key={b.start}
                      className="relative h-full flex items-end justify-center min-w-0"
                    >
                      <div
                        tabIndex={0}
                        role="button"
                        ...
                        onFocus={(e) => {
                          const tt =
                            e.currentTarget.querySelector<HTMLElement>(".tt");
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="block whitespace-nowrap">
                            Entries: {formatNumber(b.count)}
                          </span>
                          <span className="block whitespace-nowrap">
                            Cumulative: {formatNumber(b.cumulative)}
                          </span>
                        </div>
                      </div>
                        ...
                        <span
                          ...
                          style={{
                            left:
                              i === 0
                                ? "0"
                                : i === data.chartBuckets.length - 1
                                  ? "auto"
                                  : "50%",
                            right:
                              i === data.chartBuckets.length - 1 ? "0" : "auto",
                            bottom: "-36px",
                            transform:
                              i === 0 || i === data.chartBuckets.length - 1
                                ? "none"
                                : "translateX(-50%)",
